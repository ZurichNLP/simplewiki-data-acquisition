#!/bin/bash
#SBATCH --time=01:00:00
#SBATCH --cpus-per-task=1
#SBATCH --mem=16G
#SBATCH --gres=gpu:1
#SBATCH --qos=vesta
#SBATCH --partition=volta

# calling script needs to set:
# $REPO

REPO=$1

TRANSFORMERS=$REPO/transformers
BASE_TRANSFORMER=$TRANSFORMERS/base_transformer

cd $BASE_TRANSFORMER

DATA_BIN=$BASE_TRANSFORMER/data-bin
CHECKPOINT_DIR=$BASE_TRANSFORMER/checkpoints/checkpoints_de_ls_base_transformer

GENERATION_OUT=$BASE_TRANSFORMER/generation
REFERENCE=$REPO/data/de_ls/test.ls
MOSES=$TRANSFORMERS/software/mosesdecoder

fairseq-generate $DATA_BIN/simplewiki_de_ls \
    --path $CHECKPOINT_DIR/checkpoint_best.pt \
    --max-tokens 4096 \
    --beam 5 \
> $GENERATION_OUT/test.out

echo "Postprocessing..."
grep -P '^H-' $GENERATION_OUT/test.out \
| sed "s/\@\@ //g" \
| perl $MOSES/scripts/tokenizer/detokenizer.perl -l de -q \
| awk -F'H-' '{print $2}' \
| sort -n \
| cut -f 3 \
> $GENERATION_OUT/test.ls.translated

echo "Calculating BLEU"
cat $GENERATION_OUT/test.ls.translated | sacrebleu $REFERENCE
