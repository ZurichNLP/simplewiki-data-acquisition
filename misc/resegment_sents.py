...
def parse_args(): # python3 resegment_sents.py --input-tsv simplede_all.tsv --output-tsv simplede_all.resegmented.tsv --language de
    ...
    parser.add_argument('--input-tsv', type=argparse.FileType('r'), metavar='PATH',
                        help='input tsv with old segmentation')
    parser.add_argument('--output-tsv', type=argparse.FileType('w'), metavar='PATH',
                        help='output tsv with new segmentation')
    parser.add_argument('-l', '--language', type=str, metavar='LANG', default='de',
...
def create_new_segmentation(infile: IO, outfile: IO, lang: str):
    ...
    simple_writer = csv.DictWriter(outfile, simple_fieldnames, delimiter='\t', extrasaction='ignore')
    ...
    with MosesSentenceSplitter(lang) as splitsents:
        for line in simple_reader:
            ...
            if (last_article_id != line['simple_article_id'] or last_section_id != line['simple_section_id']) and last_section_id != None:
                sent_id = 1
                if section_content.strip():
                    for sent in splitsents([section_content]):
                        out_line = last_line
                        out_line.update({"simple_sent": "NOT_PARALLEL", "simplede_sent": sent, "simple_sent_id": sent_id})
